...
cat package.json.template \
	| jq --arg SERVICE "$INPUT_SERVICE_NAME" \
	     --arg REGISTRY "$INPUT_REGISTRY_NAMESPACE" \
	     '.name="@\($REGISTRY)/\($SERVICE)-client"' \
	| jq --arg SERVICE "$INPUT_SERVICE_NAME" \
	     '.description="Autogenerated API client for \($SERVICE) service."' \
	| jq --arg SERVICE "$INPUT_SERVICE_NAME" \
	     --arg REGISTRY "$INPUT_REGISTRY_NAMESPACE" \
	     '.repository.url="ssh://git@github.com/\($REGISTRY)/\($SERVICE).git"' \
	| jq --arg VERSION "$DATE" \
	     '.version="0.0.\($VERSION)"' \
	> package.json
