...
cat package.json.template | jq --arg p1 "$INPUT_SERVICE_NAME" --arg p2 "$INPUT_REGISTRY_NAMESPACE" '.name="@($p2)/\($p1)-client"' | jq --arg pn "$INPUT_SERVICE_NAME" '.description="Autogenerated API client for \($pn) service."' | jq --arg p1 "$INPUT_SERVICE_NAME" --arg p2 "$INPUT_REGISTRY_NAMESPACE" '.repository.url="ssh://git@github.com/($p2)\($p1).git"' | jq --arg date "$DATE" '.version="0.0.\($date)"' > package.json
if [ $? -ne 0 ]
then
...
cd old_version
npm init --yes
npm i @$INPUT_REGISTRY_NAMESPACE/$INPUT_SERVICE_NAME-client
if [ $? -eq 0 ]
then
  echo "[SUCCESS]"
  ...
  cksum old_version/node_modules/@$INPUT_REGISTRY_NAMESPACE/$INPUT_SERVICE_NAME-client/*.js | awk '{print $1":"$2}' >> old_checksums
  cksum old_version/node_modules/@$INPUT_REGISTRY_NAMESPACE/$INPUT_SERVICE_NAME-client/*.d.ts | awk '{print $1":"$2}' >> old_checksums
  cksum old_version/node_modules/@$INPUT_REGISTRY_NAMESPACE/$INPUT_SERVICE_NAME-client/models/** | awk '{print $1":"$2}' >> old_checksums
